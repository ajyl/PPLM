...
def perturb_past(
    ...
    for i in range(num_iterations):
        ...
        loss = torch.zeros(last.shape[0]).to(last.device)
        loss_list = []
...
def full_text_generation(
    ...
    unpert_gen_tok_text, _, _ = generate_batch(
        model,
        tokenizer,
        config,
        context=context,
    ...
    for i in range(num_samples):
        pert_gen_tok_text, discrim_loss, loss_in_time = generate_batch(
            model,
            tokenizer,
            config,
            context=context,
...
def generate_batch(
    ...
    # [batch, seq_len]
    output_so_far = context.to(config.device).long()
    ...
    #_output = model(output_so_far, attention_mask=mask)
...
def run_pplm_batch(
    ...
    unpert_gen_tok_text, pert_gen_tok_texts, _, _ = full_text_generation(
        model,
        tokenizer,
        tokenized_prompts["input_ids"].to(config.device),
        tokenized_prompts["attention_mask"].to(config.device),
        classifier,
        class_id,
        config,
    )
